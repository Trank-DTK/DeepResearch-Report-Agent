...
本系统面向需要快速获得「有来源、有图表」调研报告的使用者（学生、开发者、研究者）。它不依赖 LangChain 等 Agent 框架，而是自行实现了一个最小可用的 Agent 内核：由大模型负责规划与写作决策，由程序负责检索、绘图、校验与装配，二者通过一套 JSON 动作协议协作。

系统在设计上参考了长图文报告生成方向的研究工作（见文末「致谢」），并把这些思路落成了可运行的工程实现：规划式提纲、多源证据检索、**FDV（Find-Draw-Verify）图表生成范式**、图文一致性校验与报告质量自检。
...
| 能力 | 说明 |
|---|---|
| 手写 Agent 内核 | 不依赖 LangChain/LangGraph；自行实现「规划 + JSON 动作循环」（thought / action / args），含步数上限、未知动作回灌纠错、工具异常隔离、章节级熔断与保底成稿 |
...
| FDV 图表管线 | Find（从证据提取数值）→ Draw（绘图）→ Verify（图文一致性校验）；图型由代码按数据形态决策（占比→环形图、趋势→折线、并列对比→柱状/条形），配色随图序变化 |
| 报告自检器 | 合成后自动体检：引用完整性、引用越界（防幻觉）、图表一致性、章节结构、字数达标，输出 ✅/⚠️/❌ 清单 |
| 插件化扩展 | `src/providers/` 目录自动发现：新增检索器、模型渠道、校验器只需复制模板并实现一个方法 |
| 双渠道模型 | 同一套 OpenAI 兼容适配层，本地（Ollama / LM Studio）与在线 API（DeepSeek 等）通过配置切换 |
| Web 界面 | Streamlit：输入问题、实时进度日志、Markdown/HTML 报告预览与一键下载 |
...
| 层次 | 模块 | 职责 |
|---|---|---|
| UI 层 | `src/ui/app.py` | Streamlit 界面：问题输入、配置、进度展示、报告预览与下载 |
...
| | `src/report/` | FDV 图表、TextVerifier、报告合成、HTML 渲染、质量自检器 |
...
研究问题
  → 规划提纲（Planner）
  → 逐章执行动作循环：search（多源检索+抓取+入库）→ draw_chart（FDV）→ write_section
...
- **质量自检**：输出形如「共 7 项：✅7 ⚠️0 ❌0」的体检清单，越界引用等问题会被明确标出
- **文件**：`data/outputs/report_*.md` 与 `report_*.html`（HTML 在浏览器中可直接打印为 PDF）
