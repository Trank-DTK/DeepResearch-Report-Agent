...
- ✅ **D15 验收通过**：render_chart 三类型 PNG 生成（32/47/37KB）+ 三个坏数据用例全被优雅拒绝（错误可读）；中文渲染经视觉桥逐字验证无乱码。图存 `data/chart/`（用户自定单数目录）。**当前 D16 进行中**：draw_chart Agent 工具 + TextVerifier（verify.py，fail-open 放行设计）+ RunState.charts 字段（每章≤2 图）+ AgentContext.chart_cfg（必填）+ config.yaml chart.output_dir + AGENT_SYSTEM_PROMPT 增工具 4 说明（数据须来自证据真实数字、图行插入正文）；smoke_draw_tool 用桩 ctx 直调工具验证。
