...
- ✅ **D15/D16 验收通过**：render_chart 三类型 PNG + 坏数据拦截 + 中文无乱码（视觉桥验证）；draw_chart 工具冒烟全绿（合法出图/坏数据拒绝/每章 2 图上限/state 记录）。**排雷 2 项（用户修复中）**：①`ctx.chart_cfg.get("verify_model",...)` 拼写错误应为 `verify_mode`（否则将来切 vision 模式会静默失效）②绘图失败分支丢失 err 详情（建议 `f"绘图失败：{err}"`）。**当前 D17 进行中**：md→HTML 渲染（markdown 库 + 中文报告 CSS 模板 + 图片相对路径按项目根解析并 base64 内嵌为 data URI，单文件自包含）；m2_report 落盘 md 后顺带渲染 html；验收：浏览器打开样式正常、图不裂、Ctrl+P 可打印。
