...
- ✅ **README 与 LICENSE 已重写（AI 维护文档，非代码）**：`README.md` 全面改写——去掉大量 emoji、架构部分改为表格+数据流描述（**弃用 ASCII 边框框图**，观感更整洁）、补充特性表/目录结构/快速开始/配置说明/输出示例/运行建议/后续计划/软著与致谢；徽章数字核对为 **2274 行 / 44 文件**。`LICENSE`（MIT）已创建，**其中 `Copyright (c) 2026 <你的姓名>` 需用户替换为真实姓名**。截图已从 `soft_copyright\截图\` 复制 4 张到 `docs\images\`（01_main / 02_running / 03_report / 04_checker），README 引用仓库内路径（`soft_copyright/` 被 gitignore，不能直接引用）。
- 📌 **措辞决策（软著视角）**：原「已知限制」章节改为「运行建议」，措辞中性化（本地模型=调试配置选择、站点不可达=自动降级兜底、PDF=设计上由 HTML 打印导出），**避免"章节可能失败/功能受限"这类暗示软件不完整的表述**。原因：README 本身不属软著材料，但①它可能被审查方或者导师看到 ②说明书撰写也须遵循同一原则——只描述已实现功能，不写未实现能力。用户若在说明书里写「支持直接导出 PDF」「支持用户登录」等未实现功能，会与代码不符，属驳回雷区。
...
- ✅ **D19~D23 验收通过（W3+ 收官）**：GUI 硬 bug 清零；auto_illustrate 自动配图落地；**D22 全局引用+文末参考文献验收通过**（consolidate_reference 正确：URL 去重/首现编号/正则重写/文末"- [id]..."列表；checker 改全局集合检查；app 加"展开全文"toggle）；**D23 篇幅增强 + D23b 稳定性回调**：经验教训——**qwen3:8b 单次 JSON 长输出稳定性边界 ≈450 字**（闸门 500 导致失败率上升；回调至 400、证据截断 600、prompt "尽量分节"留合法出口后失败率显著下降）；**定位决策：本地小模型=开发调试主力，终稿/演示切在线 API（DeepSeek 等）**。用户反馈三项收尾：①篇幅（D23 处理，残余失败率可接受）②引用（D22）③美观 backlog。**当前 D24 进行中**：回归样例集——examples/questions.json（3 题：技术对比/趋势综述/数据密集）+ scripts/regression.py（逐题调 run_full_report、产物存 examples/outputs/、stats 记录章节/字数/自检/耗时、支持 --limit）；评测集雏形=科研消融实验前身。
- ✅ **D24 验收通过（回归样例集）**：3 题全部跑通并落 stats（5章7869字/6✓1⚠/652s；5章9142字/6✓/320s；6章10467字/6✓1✗/829s）→ `examples/outputs/regression_stats.json` 成为 v1.0 质量基线。**修掉 2 个模板 bug**：①`verify.py` 对含 JSON 示例的 prompt 调 `.format()` → KeyError '"consistent"' → 图表校验从上线起静默 fail-open 从未生效（教训：温和失败会藏 bug；prompt 含花括号禁用 .format）②`regression.py` 相对路径 `.relative_to(ROOT)` 崩 + `row['section']` 笔误。**校验修复后已真实生效**（日志出现"自动配图校验未通过"）。遗留改进项：auto_chart 的 `verified` 字段应取 verdict 真实值（现恒 True，致 checker 该告警失效）；残余失败率（第3题第2章 JSON 重试耗尽致正文缺失）→ 既定对策：成品演示/软著截图切 DeepSeek。**下一步 D25 软著材料包**（源程序导出脚本 + 说明书 + 截图清单 + DeepSeek 成品演示）。
