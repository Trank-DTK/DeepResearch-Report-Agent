...
- ✅ **D19~D21 验收通过（W3 收官）**：GUI 硬 bug 清零（渲染顺序修复：轮询挪到控件渲染前，弹出 RESULT 那帧按钮即恢复；QueueLogHandler super()、root INFO、error 钉住均修复）；auto_illustrate 自动配图落地（runner 内 Find→Draw→Verify→注入，不依赖 LLM 自觉）；用户实测三项反馈：①每章篇幅偏少 → D23 立项（闸门 300→500 + prompt 3~5 小节展开）②**引用编号改为全文统一+文末参考文献**（用户明确偏好，推翻按章编号决策）→ **D22 进行中**：consolidate_references（URL 全报告去重→首次出现编号→正则重写各章 [n]）+ assemble_report 删按章清单改文末"## 参考文献" + runner 在 assemble 前调用 + checker 引用检查改全局集合成员；验收=正文全局连续编号、同 URL 跨章单号、文末参考文献与正文对应 ③前端美观性 backlog（v2/GUI 打磨，非主线）。
