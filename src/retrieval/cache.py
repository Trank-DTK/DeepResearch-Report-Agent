...
class SearchCache:
  ...
  def _get_file_path(self,query:str,max_results:int) -> Path:
    filename = f"{self._key(query,max_results)}.json"
    return self.cache_dir / filename
  
  def get(self,query:str,max_results:int) -> list[SearchResult] | None:
    """从缓存中读取数据"""
    file_path = self._get_file_path(query,max_results)
...
def cached_search(provider:SearchProvider,cache:SearchCache,query:str,max_results:int=5) -> list[SearchResult]:
  ...
  cache.set(query,max_results,res)
  return res